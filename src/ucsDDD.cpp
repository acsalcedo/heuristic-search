...
// CONVERT THE STRING TO A STATE
   ...
   priority_queue<Node*, std::vector<Node*>, CompareCosts> pq;
   ...
   while (!pq.empty()) {
      ...
      while((ruleid = next_ruleid(&iter)) >= 0) {
         ...
         dist = n->getCost + get_fwd_rule_cost(ruleid);

         if (!n->getColor) {

            n->setColor(-1);
            n->setCost(dist);
            n->makeNode(child,get_fwd_rule_label(ruleid));
         
         }  else if (dist < n->getCost()) {
            assert(n->getColor == -1);
            n->setCost(dist);
            n->set
